...
// Test get_modules returns empty list initially
TEST_F(PyModuleRegistryTestHelper, GetModulesEmpty) {
  auto modules = registry->get_modules();
  ASSERT_TRUE(modules.empty());
}

// Test get_module returns empty reference for non-existent module
...
// Test client registration with replace
TEST_F(PyModuleRegistryTestHelper, RegisterClientReplace) {
  ...
  // Register second client with same name, no replace
